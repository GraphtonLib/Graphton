...
export default class GenerateCommand {
    ...
    async generate(schemaUri, options) {
        ...
        const types = this.gqlSchema.types.filter((type) => !type.name.startsWith("__") && ignoreTypes.indexOf(type.name) < 0);
        ...
        const scalarTypes = types.filter((type) => type.kind === "SCALAR" && Object.keys(baseScalars).indexOf(type.name) < 0);
    ...
    *generateQueryFactory(rootType, queries, exportFactoryAs, executionFunctionName = null) {
        ...
        for (const query of queries) {
            ...
            yield fillStub("Query", {
                ...
                RETURNTYPEBUILDER: returnTypeInfo?.kind == "object" ? `${this.returnTypeInfo(query.type)?.type}ReturnTypeBuilder` : "null",
            }, includeInStub);
            yield "";
        }
    }
    ...
    toTypeAppend(type, isRootType = true, enumsAreStrings = true) {
        ...
        if (!typeInfo.notNull) {
            ...
            typeAppend += `${isRootType ? "?" : ""}: ${enumsAreStrings && typeInfo.kind === "enum"
